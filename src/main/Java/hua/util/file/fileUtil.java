...
public class fileUtil {
    ...
    public static Map upFile(HttpServletRequest request,String user_id) throws FileNotFoundException {
        //上传的文件存放在项目的target目录
        String filePathDir = ResourceUtils.getURL("classpath:").getPath().split("classes")[0]+"file/";
        //1.创建文件对象，配置缓存信息
            ...
            for (FileItem item:items){
                if(item.isFormField()) {
                    ...
                    if(suffix.toLowerCase().equals(".doc")||suffix.toLowerCase().equals(".docx")||suffix.toLowerCase().equals(".pdf")){
                        ...
                        map.put("is",true);
                        map.put("msg","文件上传成功");
                        map.put("fileName","http://192.168.1.72:8089/file/"+fileName);
                        return map;
                    }
                    else{
                        map.put("is",false);
                        map.put("msg","请上传规定的文件类型(word、pdf)");
                        return map;
                    }
                }
            }
